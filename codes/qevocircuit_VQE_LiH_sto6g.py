...
file = open('LiH-molecules-sto6g-2000.txt', 'r').readlines()
basis_set = 'sto6g'
#basis_set = '631g'
...
env.set_filename(f"{num_qubits}qubits_{num_points}points_{num_circuit}circuits_{depth}depth_{num_generation}generations_VQE_LiH_{basis_set}_fitness_{localtime.tm_year}-{localtime.tm_mon}-{localtime.tm_mday}_1000")

# Automatically save the results in the same level folder
env.evol(1)

computation_value = vqe.general_VQE(env.best_circuit, lih([0, .25]), basis=basis_set)
