...
file = open('H4-chain-molecules-sto6g-2000.txt', 'r').readlines()
...
def VQE_fitness(qc: qiskit.QuantumCircuit, atom: str, basis: str) -> float:
    ...
    exact_value = eval(file[round((eval(atom.split(' ')[-1])/3-0.25)/((2.5-0.25)/1999))].split(" ")[-2])
...
def VQE_H4_chain_sto6g_fitness(qc: qiskit.QuantumCircuit) -> float:
    ...
    list_distances = [[_] for _ in np.linspace(0.25,2.5, num_points)]
    fitnesss = []
    # Run for num_points
...
env = EEnvironment(
    ...
    fitness_func=VQE_H4_chain_sto6g_fitness,
...
env.set_filename(f"{num_qubits}qubits_{num_points}points_{num_circuit}circuits_{depth}depth_{num_generation}generations_VQE_H4_chain_{basis_set}_fitness_{localtime.tm_year}-{localtime.tm_mon}-{localtime.tm_mday}_1000")
