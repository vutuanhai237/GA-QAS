...
import sys
sys.path.insert(0, '..')
import qoop.qcompilation, qoop.ansatz
...
def test_onequbit_tomography():
    ...
    compiler = qoop.qcompilation.QuantumCompilation(
        u = qcu3,
        vdagger = qoop.ansatz.zxz_layer(1).inverse(),
        optimizer = 'adam',
...
def test_nqubit_tomography():
    ...
    num_layers = 1
    compiler = qoop.qcompilation.QuantumCompilation(
        u = qoop.core.state.create_ghz_state(num_qubits).inverse(),
        vdagger = qoop.ansatz.Wchain_ZXZlayer_ansatz(num_qubits, num_layers),
        optimizer = 'adam',
        loss_func='loss_fubini_study'
    )
    compiler.fit(num_steps=100, verbose = 1)
